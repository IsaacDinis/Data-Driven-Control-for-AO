...
def plot_int_dd_comp(img_int,img_dd,D,pupil_diam,lamb,step,pad):
    psf_size = int(pupil_diam/step+2*pad)
    fs = pupil_diam/(step*D)
    res = fs/psf_size

    fov = 6
    axis = np.arange(-fs / 2, fs / 2, res)
    labels = np.arange(-fov,fov+1,1)
    ...
    plt.suptitle('Coronographic PSF comparison', fontsize=18)
    plt.tight_layout(pad=0.0001)

    plt.figure(figsize=(19, 14))

    plt.subplot(211)
    plt.plot(axis,img_int[int(psf_size/2),:],axis,img_dd[int(psf_size/2),:])
    plt.yscale('log')
    ...
    plt.title('PSF cut parallel to wind axis', fontsize=18)
    ...
    plt.ylabel('log10 contrast', fontsize=18)
    plt.legend(['int','dd'],fontsize=18)
    plt.title('PSF cut perpendicular to wind axis', fontsize=18)
