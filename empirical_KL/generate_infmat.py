...
if __name__ == "__main__":
    ...
    C = np.zeros((n_pix,hadamard_size))
    ...
    if bool_hadamard:
        for i in range(hadamard_size):
            supervisor.rtc.set_perturbation_voltage(0, "", hadamard_matrix[:n_act_square,i]*amp)
            supervisor.next()
            supervisor.next()
        ...
        C = C @ np.linalg.inv(hadamard_matrix)
