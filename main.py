...
def compute_strehl(phase_turb,phase_diff_limit,c_obs):
    mask = pupil_mask(phase_turb.shape[0],0.14)
...
def intensity_coro(phase,strehl,mask,step,pad,phase_factor=1,strehl_factor=1):
    phase *= phase_factor
    phase -= np.mean(phase[mask > 0])
...
def compute_psf(phase, mask, step, pad, phase_factor=1):
    phase *= phase_factor
    phase -= np.mean(phase[mask > 0])
...
def contrast():
    ...
    dummy = np.genfromtxt('phase_int/phase_int_3.csv', delimiter=",")
    dummy2 = np.genfromtxt('phase_int/tar_int_3.csv', delimiter=",")
    ...
    for i in range(3,20):
        ...
        i_coro_dd = intensity_coro(phase_dd, strehl_dd[i], mask_wfs, step, pad,3.5)
        ...
        i_coro_int = intensity_coro(phase_int, strehl_int[i], mask_wfs, step, pad,5.25)
        ...
        # phase_dd = phase_psd(phase_dd, mask_wfs, step, 50)
        i_coro_dd_tar = intensity_coro(phase_dd_tar, strehl_dd[i], mask_tar, step, pad,1,1.09)
        phase_dd_tar_tot += i_coro_dd_tar

        phase_int_tar = np.genfromtxt('phase_int/tar_int_'+str(i)+'.csv', delimiter=",")
        # phase_int = phase_psd(phase_int,mask_wfs,step,50)
        i_coro_int_tar = intensity_coro(phase_int_tar, strehl_int[i], mask_tar, step, pad,5.25)
