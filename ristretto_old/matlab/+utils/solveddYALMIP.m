...
Fx = utils.resp(tf(system.controller.Fx,1,Ts),W); % fixed part X
Fy = utils.resp(tf(system.controller.Fy,1,Ts),W); % fixed part Y
...
if ~isempty(parameters.radius)
    ...
    n = utils.getNormalDirection(Ycs_);
...
for mod =  1: nMod
    ...
    G = utils.resp(system.model(:,:,mod),W); % Model Frequency utils.response
    ...
    if  (sol.satisfyConstraints)
        ...
        W1 = utils.resp(objective.oinf.W1,W);
        W4 = utils.resp(objective.oinf.W4,W).*G;
        W14 = sqrt(abs(W1).^2+abs(W4).^2);
        if all(W14==0), F_a = [];else, F_a = W14.*Yf;end
        
        W2 = utils.resp(objective.oinf.W2,W).*G;
        W3 = utils.resp(objective.oinf.W3,W);
        
        W23 = sqrt(abs(W2).^2+abs(W3).^2);
        if all(W23==0), F_b = [];else, F_b = W23.*Xf;end
        
        CON = [CON, utils.rcone_serialized(PHI,gam,[F_a,F_b])];
        % END Hinf
        
        % H2
        gam =   0.5*gamma_2 ;
        
        W1 = utils.resp(objective.o2.W1,W);
        W4 = utils.resp(objective.o2.W4,W).*G;
        W14 = sqrt(abs(W1).^2+abs(W4).^2);
        if all(W14==0), F_a = [];else, F_a = W14.*Yf;end
        
        W2 = utils.resp(objective.o2.W2,W).*G;
        W3 = utils.resp(objective.o2.W3,W);
    ...
    if (parameters.robustNyquist)
        ...
        n = utils.getNormalDirection(Pc_);
    ...
    W1 = utils.resp(constraints.W1,W);
    W4 = utils.resp(constraints.W4,W).*G;
    W14 = max(abs(W1),abs(W4));
    if all(W14==0), F = [];else, F = W14.*Yf;end
    
    CON = [CON, utils.rcone_serialized(PHI,gam,F)];
    
    gam = 0.5*ones(nCon,1)*(1+slack(2));
    
    W2 = utils.resp(constraints.W2,W).*G;
    W3 = utils.resp(constraints.W3,W);
