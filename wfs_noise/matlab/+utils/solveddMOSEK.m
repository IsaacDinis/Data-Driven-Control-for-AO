    ...
    if (~isempty(objective.o2.W1) || (~isempty(objective.o2.W4)))
        ...
        for mod =  1: nMod
            ...
            if ~isempty(objective.o2.W1)
                W1 = utils.resp(objective.o2.W1,system.W);
            else
                W1 = zeros(nCon,1);
            end
            if ~isempty(objective.o2.W4)
                W4 = PLANT(:,mod).*utils.resp(objective.o4.W1,system.W);
            else
                W4 = zeros(nCon,1);
            end
    ...
    if (~isempty(objective.oinf.W1) || (~isempty(objective.oinf.W4))) 
        ...
        for mod =  1: nMod
            ...
            if ~isempty(objective.oinf.W1)
                W1 = utils.resp(objective.oinf.W1,system.W);
            else
                W1 = zeros(nCon,1);
            end
            if ~isempty(objective.oinf.W4)
                W4 = PLANT(:,mod).*utils.resp(objective.oinf.W1,system.W);
            else
                W4 = zeros(nCon,1);
            end
    ...
    if (~isempty(objective.o2.W2) || (~isempty(objective.o2.W3)))
        ...
        for mod =  1: nMod
            ...
            if ~isempty(objective.o2.W3)
                W3 = utils.resp(objective.o2.W3,system.W);
            else
                W3 = zeros(nCon,1);
            end
            if ~isempty(objective.o2.W2)
                W2 = utils.resp(objective.o2.W2,system.W).*PLANT(:,mod);
            else
                W2 = zeros(nCon,1);
            end
    ...
    if (~isempty(objective.oinf.W2) || (~isempty(objective.oinf.W3)))
        ...
        for mod =  1: nMod
            ...
            if ~isempty(objective.oinf.W3)
                W3 = utils.resp(objective.oinf.W3,system.W);
            else
                W3 = zeros(nCon,1);
            end
            if ~isempty(objective.oinf.W2)
                W2 = PLANT(:,mod).*utils.resp(objective.oinf.W2,system.W);
            else
                W2 = zeros(nCon,1);
            end
    ...
    if (~isempty(constraint.W1) || ~isempty(constraint.W4))
        ...
        for mod =  1: nMod
            ...
            if ~isempty(constraint.W1)
                W1 = abs(utils.resp(constraint.W1,system.W));
            else
                W1 = zeros(nCon,1);
            end
            if ~isempty(constraint.W4)
                W4 = abs(PLANT(:,mod).*utils.resp(constraint.W1,system.W));
            else
                W4 = zeros(nCon,1);
            end
            cW1_4(:,mod) = max(W1,W4);
    ...
    if (~isempty(constraint.W2) || ~isempty(constraint.W3))
        ...
        for mod =  1: nMod
            ...
            if ~isempty(constraint.W2)
                x3_d1 = PLANT(:,mod).*utils.resp(constraint.W2,system.W);
            else
                x3_d1 = zeros(nCon,1);
            end
            
            if ~isempty(constraint.W3)
                x3_d2 = utils.resp(constraint.W3,system.W);
            else
                x3_d2 = zeros(nCon,1);
            end
...
if ~isempty(parameters.radius)
    ...
    Ycs_= Zy_*(Y_c);
    dY = utils.getNormalDirection(Ycs_);
    
    x1_a = 2*real(conj(dY).*Zy_(2:end,:));
    x1_b = 2*real(conj(dY).*Zy_(1:end-1,:));
    
    M.constraint(Expr.add(Expr.constTerm(x1_a*Y_c),Expr.mul(Matrix.dense(x1_a),Y)),Domain.greaterThan(1e-5));
    M.constraint(Expr.add(Expr.constTerm(x1_b*Y_c),Expr.mul(Matrix.dense(x1_b),Y)),Domain.greaterThan(1e-5));
end
...
for mod =  1: nMod
    ...
    if  (parameters.robustNyquist)
        ...
        dP = utils.getNormalDirection(Pc_);
